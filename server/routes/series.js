...
function getDefaultQualityProfileName() {
  const row = db.prepare("SELECT data FROM app_settings WHERE section = 'library-defaults'").get();
  if (!row) return null;
  let defaultId;
  try {
  ...
  const profileRow = db.prepare("SELECT data FROM settings_items WHERE section = 'profiles' AND id = ?").get(defaultId);
...
// feature existed.
function resolveQualityProfile(requestedName) {
  if (requestedName) return requestedName;
  const defaultName = getDefaultQualityProfileName();
  if (defaultName) return defaultName;
  const firstRow = db.prepare("SELECT data FROM settings_items WHERE section = 'profiles' ORDER BY position ASC, id ASC LIMIT 1").get();
...
function getDefaultIgnoreSpecials() {
  const row = db.prepare("SELECT data FROM app_settings WHERE section = 'library-defaults'").get();
  if (!row) return false;
  try {
...
// default.
function resolveIgnoreSpecials(requested) {
  if (requested !== undefined && requested !== null) return requested ? 1 : 0;
  return getDefaultIgnoreSpecials() ? 1 : 0;
}
...
if (seriesCount === 0 && SEED_DEMO_DATA) {
...
async function handleSeriesApi(req, res, urlPath) {
  ...
  if (req.method === 'POST' && urlPath === '/api/series') {
    ...
    // own comment.
    const qualityProfile = resolveQualityProfile(body.qualityProfile ? String(body.qualityProfile) : null);
    ...
    const ignoreSpecials = resolveIgnoreSpecials(body.ignoreSpecials);

    const { lastInsertRowid } = db.prepare(`
      INSERT INTO series (title, badge, fill, pct, eps, monitored, status, air_status, next_air_days, added_days_ago, poster, meta, overview, external_id, external_source, alt_titles, path, quality_profile, ignore_specials)
      VALUES (?, NULL, 'accent', 0, '0 / 0', 1, ?, ?, NULL, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(title, status, airStatus, body.poster || null, meta, body.overview || null, externalId, externalSource, altTitlesJson, defaultPath, qualityProfile, ignoreSpecials);
