...
function insertBlocklistRow({ seriesId, episodeId, releaseTitle, reason, indexer }) {
  db.prepare(`
    INSERT INTO blocklist (series_id, episode_id, release_title, reason, indexer)
    VALUES (?, ?, ?, ?, ?)
  `).run(seriesId ?? null, episodeId ?? null, releaseTitle, reason ?? null, indexer ?? null);
}
